...
# from https://github.com/chilicuil/learn
...
def convert_macaddress(macaddress):
    converted = ''
    for i in range(0, 6):
        # add colon every 2 characters
        converted = converted + macaddress[0:2] + ':'
        macaddress = macaddress[2:]
    # return uppercase, remove last colon
    return converted[:-1].upper()
...
if __name__ == "__main__":
    ...
    while 1: #main loop
        try:
            ...
            data, dhcpopt=reqparse(message) #handle request
            if data:
                s.sendto(data,('<broadcast>',68)) #reply
            release() #update releases table
            if dhcpopt == '350103': # hack the script to exit on lease ACKed
                exit(0)
